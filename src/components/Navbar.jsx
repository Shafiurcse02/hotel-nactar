...
const Navbar = ({ links }) => {
  ...
  return (
    <React.Fragment>
      <AppBar position="sticky" sx={{ backgroundImage: "radial-gradient(circle, rgba(63,94,251,1) 23%, rgba(252,70,113,1) 100%);" }}>
          ...
          {!isMatch && <Grid sx={{ placeItems: "center" }} container>
            ...
            <Grid item sx={5} md={5}>
              <Box display={"flex"}>
                ...
                {links.map((l) => (
                  <MenuItem key={l} onClick={handleCloseUserMenu} >
                     <Typography onClick={(e,val)=>navigate(`/${l}`)}>
                                {l}
                            </Typography>
                  </MenuItem>
                ))}
